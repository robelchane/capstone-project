...
export default function PropertyPage({ params }) {
  ...
  return (
    <main className="font-serif  text-black p-8 flex flex-col lg:flex-row mt-12 gap-8">
      ...
      <div className="lg:w-2/3 w-full pr-8">
        ...
        <div className="my-8">
          ...
          <div className="h-64 rounded-lg overflow-hidden">
            ...
            <Map lat={property.latitude} lng={property.longitude} key={property.id}/>
          </div>
        </div>
      </div>
      ...
      <div className="mt-20 w-1/3 p-6 rounded-lg h-full ">
        <ContactOwner property={property} />
        <div>
        <MortgageCalculator property={property.price} />
        </div>
      </div>
    </main>
  );
